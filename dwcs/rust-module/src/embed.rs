...
fn run_transformer(encoding: &Encoding) -> [f32; EMBED_DIM] {
    ...
    assert_eq!(
        &w[c..c + 4],
        b"MLM2",
        "weights magic mismatch (stale/old-format weights.bin?)"
    );
    c += 4;
...
fn transformer_layer(
    ...
    let q: Vec<Vec<f32>> = hidden
        .iter()
        .map(|h| matmul_row_bias(h, &q_w, &q_b, hidden_size))
        .collect();
    let k: Vec<Vec<f32>> = hidden
        .iter()
        .map(|h| matmul_row_bias(h, &k_w, &k_b, hidden_size))
        .collect();
    let v: Vec<Vec<f32>> = hidden
        .iter()
        .map(|h| matmul_row_bias(h, &v_w, &v_b, hidden_size))
        .collect();
